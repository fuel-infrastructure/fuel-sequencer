...
func (h *FuelSequencerProposalHandler) generateMsgIndexAndEventTxs(
	...
	// If an Authorize event cannot be encoded as bytes tx or not authenticated we will skip it. This is done to protect
	// the Sequencer from attacks from users that submit invalid encodings. On the other hand, deposit events are
	// expected to always have the same structure as they are fully generated by the smart contracts. Due to this, we
	// will halt the block production if a deposit event fails to be encoded as a bytes tx.
