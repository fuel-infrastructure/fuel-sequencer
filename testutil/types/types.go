...
func MustGetDepositMsgFromDepositEvent(
	cdc codec.BinaryCodec, authority, denom string, depositEvent *sidecartypes.Event,
) *bridgetypes.MsgDepositFromEthereum {

	msgs, err := depositEvent.Messages(cdc, authority, denom)
	if err != nil {
		panic(err)
	} else if len(msgs) > 1 {
...
func MustGetEventTxsFromEvents(
	cdc codec.BinaryCodec, authority, denom string, events []*sidecartypes.Event, eventTxsSequence uint64,
) (allRawTxBytes [][]byte) {

	for _, event := range events {
		rawTxBytes, err := event.RawTxBytes(cdc, authority, denom, eventTxsSequence)
		if err != nil {
			panic(err)
		}
...
func MustGetSizeFromEvents(cdc codec.BinaryCodec, authority, denom string, events []*sidecartypes.Event, sequence uint64) int {
	return int(utils.TxsSize(MustGetEventTxsFromEvents(cdc, authority, denom, events, sequence)))
}
