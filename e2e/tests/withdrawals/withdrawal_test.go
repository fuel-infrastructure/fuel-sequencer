...
func (s *WithdrawalsTestSuite) TestWithdrawalWithFuelStreamXManualProcess() {
	...
	s.Run("Submit a withdrawal on the Sequencer and make sure it can be actioned on Ethereum", func() {
		...
		// UPDATE_DELAY_BLOCKS has to be greater than the height at which the withdrawal result resides, so that we are
		// sure that the withdrawal result will get captured in the bridge commitment.
		s.Require().Greater(int64(testsuite.UPDATE_DELAY_BLOCKS), withdrawalResponse.Height)

		// We need to wait some blocks so that we're at a height that is greater than the operator UPDATE_DELAY_BLOCKS,
		// so that we are sure that the bridge commitment can capture the withdrawal result when we run FuelStreamX
		err = s.WaitUntilSequencerBlock(s.Ctx(), testsuite.UPDATE_DELAY_BLOCKS+1, time.Minute)

		startBlockString, targetBlockString, _, _, receipt := s.RunFuelStreamXManualProcess()
