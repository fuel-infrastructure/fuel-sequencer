...
func (h *FuelSequencerProposalHandler) ProcessProposalHandler() sdk.ProcessProposalHandler {
	return func(ctx sdk.Context, req *abci.RequestProcessProposal) (*abci.ResponseProcessProposal, error) {
		...
		//
		// The proposer could have given us an incorrect NumInjectedEventTxs, which is why we include them in the error.
		...
		if err != nil {
			return &abci.ResponseProcessProposal{Status: abci.ResponseProcessProposal_REJECT}, fmt.Errorf(
				"failed to trim event txs from tail (proposer: %s): %w", sdk.ConsAddress(req.ProposerAddress), err,
			)
		}
		if trimmed > 0 {
			ctx.Logger().Debug(fmt.Sprintf(
		...
		// Extract the injected event txs that we have so far and ensure that we have the right amount of transactions.
		//
		// The proposer could have given us an incorrect NumInjectedEventTxs, which is why we include them in the error.
		...
		if uint64(len(injectedEventTxs)) != injectedMsgIndex.NumInjectedEventTxs {
			return &abci.ResponseProcessProposal{Status: abci.ResponseProcessProposal_REJECT}, fmt.Errorf(
				"unexpected number of injected event txs (proposer: %s); expected: %d, got: %d",
				sdk.ConsAddress(req.ProposerAddress), injectedMsgIndex.NumInjectedEventTxs, len(injectedEventTxs),
			)
		}

		// Reject block if injected MsgIndex does not match the one generated by the validator verifying the block.
		//
		// The proposer could have given us incorrect MsgIndex or event txs, which is why we include them in the error.
		err = injectedMsgIndex.Equal(msgIndex, injectedEventTxs, eventTxs)
		if err != nil {
			return &abci.ResponseProcessProposal{Status: abci.ResponseProcessProposal_REJECT}, fmt.Errorf(
				"generated injected txs do not match the ones from the block proposal (proposer: %s): %w",
				sdk.ConsAddress(req.ProposerAddress), err,
			)
		}
