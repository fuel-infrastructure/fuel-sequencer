...
func (h *FuelSequencerProposalHandler) generateMsgIndexAndEventTxs(
	...
	//
	// To protect the Sequencer from attacks induced from a large or invalid payload, we skip Authorize events if:
	...
	// This means that for deposits we are more strict and halt block production if such an event:
	...
	for _, event := range events {
		...
		if err != nil {
			...
			// already been skipped. Therefore, the error is unexpected and we should halt block production.
