...
run-sidecar:
	...
	@echo "Waiting for Ethereum node $(ETH_WS) to start..."
	@while ! curl -s -X POST -H "Content-Type: application/json" --data '{"jsonrpc":"2.0","method":"web3_clientVersion","params":[],"id":1}' --max-time 1 $(ETH_WS) | grep -q "result"; do \
	    sleep 1; \
	done
...
test-e2e: check-docker-image-exists check-eth-docker-image-exists check-fsx-docker-images-exist test-e2e-basic test-e2e-withdrawals test-e2e-events test-e2e-authorize-transactions
