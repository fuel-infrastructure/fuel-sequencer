...
func (s *E2ETestSuite) EnsureAllDockerImagesExist(imageTags ...string) error {
	for _, tag := range imageTags {
		if err := s.ensureDockerImageExists(tag); err != nil {
			return err
		}
	}
	return nil
...
func (s *E2ETestSuite) ensureDockerImageExists(fuelSequencerDockerImageTag string) error {
	imageTag := fmt.Sprintf("%s:%s", s.FuelSequencerDockerImageRepo, fuelSequencerDockerImageTag)
	...
	targetCommit := s.targetBuildCommit(commitHash, fuelSequencerDockerImageTag)

	s.T().Logf("building Docker image for commit: %s", targetCommit)

	// Build the Docker image
	if err := s.buildDockerImageInTempRepo(targetCommit); err != nil {
	...
	// Delete HEAD and latest tags generated by this build process
	if fuelSequencerDockerImageTag != "HEAD" && fuelSequencerDockerImageTag != "latest" {
		headTag := fmt.Sprintf("%s:HEAD", s.FuelSequencerDockerImageRepo)
		s.dockerPool.Client.RemoveImage(headTag)
		latestTag := fmt.Sprintf("%s:latest", s.FuelSequencerDockerImageRepo)
		s.dockerPool.Client.RemoveImage(latestTag)
		s.T().Logf("🧹 Pruned HEAD and latest tags (auto-generated by build)")
	}
...
func (s *E2ETestSuite) buildDockerImageInTempRepo(targetCommit string) error {
	...
	buildCmd.Env = append(os.Environ(),
		...
		fmt.Sprintf("DOCKER_IMAGE_TAG=%s", targetCommit),
	)

	s.T().Logf("building Docker image with: make build-docker-image (target tag: %s)", targetCommit)
