...
func (s *sequencer) initValidatorConfigs() error {
	for i, val := range s.chain.Validators {
		...
		valConfig.Blobs.DataSizeBytes = 50e6 // 1 MB
		valConfig.Blobs.WaitBeforeSend = 0 * time.Second
		valConfig.Blobs.RecvBufferCapacity = 100e6  // 100 MB
		valConfig.Blobs.RecvMessageCapacity = 100e6 // 100 MB

		valConfig.P2P.SendRate = 1e9 // 1 GB/s
		valConfig.P2P.RecvRate = 1e9 // 1 GB/s

		valConfig.P2P.MaxPacketMsgPayloadSize = 5e6 // 100 MB
