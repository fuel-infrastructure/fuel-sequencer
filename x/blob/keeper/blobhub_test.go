...
func TestBlobhubClient_Connect(t *testing.T) {
	...
	client, err := newBlobhubClient(ctx, logger, pool, testBlobhubAddress, nil, "", false, 0, nil, nil)
	require.NoError(t, err)
	...
	_, err = newBlobhubClient(ctx, logger, pool, testBlobhubAddress, nil, "", false, 0, nil, nil)
